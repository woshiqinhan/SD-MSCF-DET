...
class BlockBody_P345(nn.Module):
    def __init__(self, channels=None):
        ...
        self.blocks_scalezero3 = nn.Sequential(Conv(channels[0], channels[0], 1))
        self.blocks_scaleone3 = nn.Sequential(Conv(channels[1], channels[1], 1))
        ...
        self.blocks_scalezero4 = nn.Sequential(*[BasicBlock(channels[0], channels[0]) for _ in range(2)])
        self.blocks_scaleone4 = nn.Sequential(*[BasicBlock(channels[1], channels[1]) for _ in range(2)])

        self.downsample_scalezero4_1 = Downsample_x2(channels[0], channels[1])
        self.downsample_scaleone4_2 = Downsample_x2(channels[1], channels[2])
        self.asff_scaletwo4 = ASFF_2(inter_dim=channels[2])

    def forward(self, input0, input1, input2):
        scalezero1 = self.blocks_scalezero1(input0)
        scaleone1 = self.blocks_scaleone1(input1)
        scaletwo1 = self.blocks_scaletwo1(input2)

        scalezero1_2 = self.downsample_scalezero1_2(scalezero1)
        scaleone1_2 = self.upsample_scaleone1_2(scaleone1)

        scalezero2 = self.blocks_scalezero2(self.asff_scalezero1(scalezero1, scaleone1_2))
        scaleone2 = self.blocks_scaleone2(self.asff_scaleone1(scalezero1_2, scaleone1))
        scaletwo2 = scaletwo1

        scalezero3 = self.blocks_scalezero3(scalezero2)
        scaleone3 = self.blocks_scaleone3(scaleone2)
        scaletwo3 = scaletwo2

        scalezero3_2 = self.downsample_scalezero3_2(scalezero3)
        scaleone3_2 = self.upsample_scaleone3_2(scaleone3)

        scalezero4 = self.blocks_scalezero4(scalezero3)
        scaleone4 = self.blocks_scaleone4(self.asff_scaleone1(scalezero3_2, scaleone3))
        scaletwo4 = self.asff_scaletwo4(self.downsample_scaleone4_2(scaleone4), self.downsample_scalezero4_1(scalezero4))

        return [scalezero4, scaleone4, scaletwo4]


class BlockBody_P345_Custom(BlockBody_P345):
    def __init__(self, channels=None, block_type=BasicBlock, repeat=4):
        channels = channels or [64, 128, 256, 512]
        super().__init__(channels)
        self.blocks_scalezero2 = nn.Sequential(*(block_type(channels[0], channels[0]) for _ in range(repeat)))
        self.blocks_scaleone2 = nn.Sequential(*(block_type(channels[1], channels[1]) for _ in range(repeat)))


class BlockBody_P2345(nn.Module):
    def __init__(self, channels=None):
        super().__init__()
        channels = channels or [64, 128, 256, 512, 1024]
        self.blocks_scalezero1 = nn.Sequential(Conv(channels[0], channels[0], 1))
        self.blocks_scaleone1 = nn.Sequential(Conv(channels[1], channels[1], 1))
        self.blocks_scaletwo1 = nn.Sequential(Conv(channels[2], channels[2], 1))
        ...
        self.downsample_scalezero1_2 = Downsample_x2(channels[0], channels[1])
        self.downsample_scaleone1_3 = Downsample_x2(channels[1], channels[2])
        self.upsample_scaleone1_2 = Upsample(channels[1], channels[0], scale_factor=2)
        self.upsample_scalethree1_2 = Upsample(channels[3], channels[2], scale_factor=2)

        self.asff_scalezero1 = ASFF_2(inter_dim=channels[0])
        self.asff_scaleone1 = ASFF_2(inter_dim=channels[1])
        self.asff_scaletwo1 = ASFF_3(inter_dim=channels[2])

        self.blocks_scalezero2 = nn.Sequential(*[BasicBlock(channels[0], channels[0]) for _ in range(2)])
        self.blocks_scaleone2 = nn.Sequential(*[BasicBlock(channels[1], channels[1]) for _ in range(2)])
        self.blocks_scaletwo2 = nn.Sequential(*[BasicBlock(channels[2], channels[2]) for _ in range(2)])
        self.blocks_scalethree2 = nn.Sequential(*[BasicBlock(channels[3], channels[3]) for _ in range(2)])

        self.downsample_scalezero2_2 = Downsample_x4(channels[0], channels[2])
        self.upsample_scaletwo2_2 = Upsample(channels[2], channels[0], scale_factor=4)
        self.downsample_scaleone2_2 = Downsample_x2(channels[1], channels[2])
        self.upsample_scaletwo2_1 = Upsample(channels[2], channels[1], scale_factor=2)

        self.blocks_scalezero3 = nn.Sequential(Conv(channels[0], channels[0], 1))
        self.blocks_scaleone3 = nn.Sequential(Conv(channels[1], channels[1], 1))
        self.blocks_scaletwo3 = nn.Sequential(Conv(channels[2], channels[2], 1))
        self.blocks_scalethree3 = nn.Sequential(Conv(channels[3], channels[3], 1))
        ...
        self.blocks_scalezero4 = nn.Sequential(*[BasicBlock(channels[0], channels[0]) for _ in range(2)])
        self.blocks_scaleone4 = nn.Sequential(*[BasicBlock(channels[1], channels[1]) for _ in range(2)])
        self.blocks_scaletwo4 = nn.Sequential(*[BasicBlock(channels[2], channels[2]) for _ in range(2)])
        self.blocks_scalethree4 = nn.Sequential(*[BasicBlock(channels[3], channels[3]) for _ in range(2)])

        self.downsample_scalezero4_1 = Downsample_x2(channels[0], channels[1])
        self.downsample_scaleone4_2 = Downsample_x2(channels[1], channels[2])
        self.downsample_scaletwo4_3 = Downsample_x2(channels[2], channels[3])

        self.asff_scalethree4 = ASFF_4(inter_dim=channels[3])

    def forward(self, input0, input1, input2, input3):
        s0 = self.blocks_scalezero1(input0)
        s1 = self.blocks_scaleone1(input1)
        s2 = self.blocks_scaletwo1(input2)
        s3 = self.blocks_scalethree1(input3)

        s0_d2 = self.downsample_scalezero1_2(s0)
        s1_u2 = self.upsample_scaleone1_2(s1)

        s0 = self.blocks_scalezero2(self.asff_scalezero1(s0, s1_u2))
        s1 = self.blocks_scaleone2(self.asff_scaleone1(s0_d2, s1))
        s2 = self.blocks_scaletwo2(self.asff_scaletwo1(s1, s2, self.upsample_scalethree1_2(s3)))
        s3 = self.blocks_scalethree2(s3)

        s0_d4 = self.downsample_scalezero2_2(s0)
        s2_u4 = self.upsample_scaletwo2_2(s2)
        s1_d2 = self.downsample_scaleone2_2(s1)
        s2_u2 = self.upsample_scaletwo2_1(s2)

        s0 = self.blocks_scalezero3(s0)
        s1 = self.blocks_scaleone3(s1)
        s2 = self.blocks_scaletwo3(s2)
        s3 = self.blocks_scalethree3(s3)

        s0_d2_b = self.downsample_scalezero3_2(s0)
        s1_d2_b = self.downsample_scaleone3_2(s1)
        s1_u2_b = self.upsample_scaleone3_2(s1)
        s3_u2_b = self.upsample_scalethree3_2(s3)

        s0 = self.blocks_scalezero4(s0)
        s1 = self.blocks_scaleone4(s1)
        s2 = self.blocks_scaletwo4(s2)
        s3 = self.blocks_scalethree4(self.asff_scalethree4(s0_d2_b, s1_d2_b, s2, s3))

        return [s0, s1, s2, s3]


class BlockBody_P2345_Custom(BlockBody_P2345):
    def __init__(self, channels=None, block_type=BasicBlock, repeat=2):
        super().__init__(channels)
        self.blocks_scalezero2 = nn.Sequential(*(block_type(self.blocks_scalezero2[0].conv.conv.out_channels,
                                                            self.blocks_scalezero2[0].conv.conv.out_channels) for _ in range(repeat)))
        self.blocks_scaleone2 = nn.Sequential(*(block_type(self.blocks_scaleone2[0].conv.conv.out_channels,
                                                           self.blocks_scaleone2[0].conv.conv.out_channels) for _ in range(repeat)))


class AFPN_P345(nn.Module):
    def __init__(self, ch=(256, 512, 1024), hidc=256):
        super().__init__()
        self.block_body = BlockBody_P345([hidc, hidc * 2, hidc * 4])
        self.out_layers = nn.ModuleList([Conv(hidc, hidc, 3), Conv(hidc * 2, hidc * 2, 3), Conv(hidc * 4, hidc * 4, 3)])

    def forward(self, x):
        feats = self.block_body(*x)
        return [layer(feat) for layer, feat in zip(self.out_layers, feats)]


class AFPN_P345_Custom(nn.Module):
    def __init__(self, ch=(256, 512, 1024), hidc=256, block_type=BasicBlock, repeat=4):
        super().__init__()
        self.block_body = BlockBody_P345_Custom([hidc, hidc * 2, hidc * 4], block_type=block_type, repeat=repeat)
        self.out_layers = nn.ModuleList([Conv(hidc, hidc, 3), Conv(hidc * 2, hidc * 2, 3), Conv(hidc * 4, hidc * 4, 3)])
...
class AFPN_P2345(nn.Module):
    def __init__(self, ch=(128, 256, 512, 1024), hidc=256):
        super().__init__()
        self.block_body = BlockBody_P2345([hidc // 2, hidc, hidc * 2, hidc * 4, hidc * 8])
        self.out_layers = nn.ModuleList(
            [Conv(hidc // 2, hidc // 2, 3), Conv(hidc, hidc, 3), Conv(hidc * 2, hidc * 2, 3), Conv(hidc * 4, hidc * 4, 3)]
        )
...
class AFPN_P2345_Custom(nn.Module):
    def __init__(self, ch=(128, 256, 512, 1024), hidc=256, block_type=BasicBlock, repeat=2):
        super().__init__()
        self.block_body = BlockBody_P2345_Custom([hidc // 2, hidc, hidc * 2, hidc * 4, hidc * 8], block_type=block_type, repeat=repeat)
        self.out_layers = nn.ModuleList(
            [Conv(hidc // 2, hidc // 2, 3), Conv(hidc, hidc, 3), Conv(hidc * 2, hidc * 2, 3), Conv(hidc * 4, hidc * 4, 3)]
        )
