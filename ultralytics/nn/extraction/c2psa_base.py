...
class SEFN(nn.Module):
    ...
    def forward(self, x, spatial):
        ...
        target_H, target_W = x1.shape[2], x1.shape[3]  # 获取 21, 21

        # 2. 处理空间分支
        y = self.avg_pool(spatial)
        y = self.spatial_conv(y)

        # 3. [修复核心] 使用 interpolate 指定 size，而不是 scale_factor
        # 这样无论 21 还是 20，都会被强制拉伸回正确的尺寸
        y = F.interpolate(y, size=(target_H, target_W), mode='nearest')
...
class EDFFN(nn.Module):
    ...
    Args:
        dim: 输入通道数
        ffn_expansion_factor: FFN扩展因子，默认2
        mlp_ratio: 等同于 ffn_expansion_factor，为了兼容接口
        patch_size: FFT处理的patch大小
        bias: 是否使用偏置，默认False
    """
    def __init__(self, dim, ffn_expansion_factor=2, mlp_ratio=None, patch_size=8, bias=False):
        super(EDFFN, self).__init__()

        # 兼容逻辑：优先使用 mlp_ratio，如果没有则使用 ffn_expansion_factor
        expansion = mlp_ratio if mlp_ratio is not None else ffn_expansion_factor
        hidden_features = int(dim * expansion)

        # 使用传入的 patch_size，而不是硬编码
