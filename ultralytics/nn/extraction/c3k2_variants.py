...
class AMSCF_C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, kernel_sizes=(3, 5, 7, 9, 11), expansion=1.0,
                 with_caa=True, caa_kernel_size=11, add_identity=True, c3k=False,
        ...
        # 创新改动：引入可学习权重 alpha，用于动态调节各分支特征的响应强度
        # 初始值为 1.0，保证训练初期效果与原版一致
    ...
    def forward(self, x):
        ...
        # 3. 创新改动：自适应特征加权
        # 在 Concat 之前，利用可学习权重对每个分支进行微调
        # 这在学术上被称为 "Channel-wise Feature Calibration"
        weighted_y = [y[i] * self.alpha[i] for i in range(len(y))]

        # 4. 最后融合
        return self.cv2(torch.cat(weighted_y, 1))
# ================================ 后续批次将在这里追加 ================================
...
def autopad(k, p=None, d=1):  # kernel, padding, dilation
    """自动计算 padding，确保输出维度一致"""
    if d > 1:
        k = d * (k - 1) + 1 if isinstance(k, int) else [d * (x - 1) + 1 for x in k]
    if p is None:
        p = k // 2 if isinstance(k, int) else [x // 2 for x in x]
    return p


class Conv(nn.Module):
    """标准卷积块：Convolution + Batch Normalization + Activation"""
...
class DynamicTanh(nn.Module):
    """
    CVPR 2025: 动态 Tanh 激活函数
    通过学习 alpha (缩放) 和 beta (偏移) 自动调整非线性响应范围
    """

    def __init__(self, chan):
        super().__init__()
        self.alpha = nn.Parameter(torch.ones(1, chan, 1, 1))
        self.beta = nn.Parameter(torch.zeros(1, chan, 1, 1))

    def forward(self, x):
        return torch.tanh(x * self.alpha + self.beta)
...
class DMSF_Unit(nn.Module):
    ...
    def __init__(self, c, kernel_sizes=(3, 5, 7), expansion=1.0, add_identity=True):
        ...
        # 1. 多尺度空间卷积分支 (通过 Groups 降低参数量)
        self.multiscale_convs = nn.ModuleList([
            nn.Conv2d(c, self.c_inner, k, padding=k // 2, groups=c) for k in kernel_sizes
        ])

        # 2. 动态统计注意力分支 (DSTA)
        self.stats_conv = nn.Conv2d(c, self.c_inner, 1)
        self.d_tanh = DynamicTanh(self.c_inner)

        # 3. 特征融合与映射
        self.proj = nn.Conv2d(len(kernel_sizes) * self.c_inner, c, 1)

    def forward(self, x):
        # 计算全局统计量 mu (ICLR 2025 ToST 核心逻辑)
        mu = torch.mean(self.stats_conv(x), dim=(2, 3), keepdim=True)

        # 提取多尺度特征并进行动态调制
        feats = []
        for conv in self.multiscale_convs:
            # 这里的交互实现了 O(n) 复杂度的注意力
            f = self.d_tanh(conv(x) * mu)
            feats.append(f)

        out = self.proj(torch.cat(feats, 1))
        return x + out if self.add_identity else out


class DMSF_C3(nn.Module):
    """
    创新模块: DMSF_C3 (替代原 MSCF_C3)
    特点: 采用 Partial-Dense 连接拓扑，内置 DMSF 动力单元
    """

    def __init__(self, c1, c2, n=1, kernel_sizes=(3, 5, 7), e=0.5):
        super().__init__()
        self.c = int(c2 * e)
        # 输入分割投影
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        # 最终特征聚合投影 (2 + n) 代表 y 列表中的 chunk(2) 以及 n 个演化层
        self.cv2 = Conv((2 + n) * self.c, c2, 1)

        # 堆叠 n 个创新单元
        self.m = nn.ModuleList(
            DMSF_Unit(self.c, kernel_sizes=kernel_sizes) for _ in range(n)
        )
    ...
    def forward(self, x):
        ...
        # 2. 密集演化 (Dense Evolution)
        # 每一层 m 处理前一层的输出，并保留所有中间结果
        for m in self.m:
            y.append(m(y[-1]))
